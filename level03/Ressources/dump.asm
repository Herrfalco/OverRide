...
0x080485f7 <+3>:	sub    $0x18,%esp							< 24 bytes for new frame

0x080485fa <+6>:	movb   $0x0,-0x9(%ebp)						< 0 in c (4 bytes)

0x080485fe <+10>:	jmp    0x8048601 <clear_stdin+13>			? while true
0x08048600 <+12>:	nop											|

0x08048601 <+13>:	call   0x80484a0 <getchar@plt>				> getchar

0x08048606 <+18>:	mov    %al,-0x9(%ebp)						< getchar result in c

0x08048609 <+21>:	cmpb   $0xa,-0x9(%ebp)						? c == '\n' || c == -1
0x0804860d <+25>:	je     0x8048615 <clear_stdin+33>			{

0x0804860f <+27>:	cmpb   $0xff,-0x9(%ebp)						|
0x08048613 <+31>:	jne    0x8048600 <clear_stdin+12>			|

0x08048615 <+33>:	leave										< return
0x08048616 <+34>:	ret											|
																}
Dump of assembler code for function get_unum:
0x08048617 <+0>:	push   %ebp									< new stack frame
0x08048618 <+1>:	mov    %esp,%ebp							|

0x0804861a <+3>:	sub    $0x28,%esp							< 40 bytes for new frame

0x0804861d <+6>:	movl   $0x0,-0xc(%ebp)						< 0 in nb (12 bytes)

0x08048624 <+13>:	mov    0x804a040,%eax						< stdout as parameter for fflush
0x08048629 <+18>:	mov    %eax,(%esp)							|
0x0804862c <+21>:	call   0x8048490 <fflush@plt>				|

0x08048631 <+26>:	mov    $0x80489c0,%eax						< "%u" and str in registers
0x08048636 <+31>:	lea    -0xc(%ebp),%edx						|
0x08048639 <+34>:	mov    %edx,0x4(%esp)						< nb as 2nd parameter for scanf
0x0804863d <+38>:	mov    %eax,(%esp)							< "%u" as 1st parameter
0x08048640 <+41>:	call   0x8048530 <__isoc99_scanf@plt>		> scanf

0x08048645 <+46>:	call   0x80485f4 <clear_stdin>				> clear_stdin

0x0804864a <+51>:	mov    -0xc(%ebp),%eax						< return nb

0x0804864d <+54>:	leave										|
0x0804864e <+55>:	ret											|

Dump of assembler code for function prog_timeout:
0x0804864f <+0>:	push   %ebp									< new stack frame
0x08048650 <+1>:	mov    %esp,%ebp							|

0x08048652 <+3>:	mov    $0x1,%eax							< 1 as parameter for exit
0x08048657 <+8>:	mov    $0x1,%ebx							|
0x0804865c <+13>:	int    $0x80								> exit

0x0804865e <+15>:	pop    %ebp									< return from function prog_timeout
0x0804865f <+16>:	ret											|
